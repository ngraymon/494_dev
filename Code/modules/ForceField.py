...
class EnergyEvaluatorParameters(object):
    ...
    def __init__(self, universe, force_field, subset1=None, subset2=None):
        ...
        if subset1 is None:
            ...
            pigs_atoms = [a for a in self.universe.atomIterator() if a.trialwave is not None]
        else:
            ...
            pigs_atoms = list(set(a for a in subset1.atomIterator() if a.trialwave is not None)
                            | set(a for a in subset2.atomIterator() if a.trialwave is not None))
        if pi_atoms:
            ...
            if pi_environments[0].include_spring_terms:
                for a in pi_atoms:
                    ...
                    for b in range(nb-1):
                        spring_parameters.append((a.index+b, a.index+(b+1), 0., k))

            if pigs_atoms:
                for a in pigs_atoms:
                    self.params.extend(a.trialwave.evaluatorParameters(self.universe,
                                                  subset1, subset2,
                                                  self.global_data))
        else:
            nbeads = 1
...
class EnergyEvaluator(object):
    ...
    def __init__(self, universe, force_field, subset1=None, subset2=None,
        ...
        if pi_atoms:
            ...
            if pi_environments[0].include_spring_terms:
                ...
                for a in pi_atoms:
                    ...
                    k = float(nbeads*nb)*a.mass() / (beta*beta*Units.hbar*Units.hbar*2.)
                    for b in range(nb-1):
                        indices.append([a.index+b, a.index+(b+1)])
        ...
        #Now that bead data exists, add trial wavefunctions!
        if pi_atoms and pigs_atoms:
                for a in pigs_atoms:
                    terms.extend(a.trialwave.evaluatorTerms(self.universe,
                                                    subset1, subset2,
                                                    self.global_data))
        ...
        self.evaluator = Evaluator(N.array(terms), 1./(nbeads-1),
