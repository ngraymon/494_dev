...
class DoubleWellTerm(EnergyTerm):
 
    def __init__(self, universe, atom_index, center, force_constant):
        EnergyTerm.__init__(self, 'double well', universe)
...
class DoubleWellForceFieldPIGS(ForceField):

    """
    Double well potential with respect to a fixed point in space
    """
    def __init__(self, atom, center, force_constant, UsingPIGS = False):
        """
    ...
    def evaluatorTerms(self, universe, subset1, subset2, global_data):
        ...
        if (self.doingPath):
            # Here we handle the potenetial energies for a PIGS approach
            f, offsets          = self.beadOffsetsAndFactor([self.atom_index], global_data)
            prefactor_array     = np.ones_like(offsets)
            M = (prefactor_array.shape[1] / 2)   # this is the middle bead
            ...
            return [DoubleWellTerm(universe, self.atom_index + offset[0], self.center, prefactor * f * self.force_constant) for offset, prefactor in zip(offsets,prefactor_array)]
        else:
            # Here we implement a classical approach to the forcefield
            return [DoubleWellTerm(universe, self.atom_index, self.center, self.force_constant)]
